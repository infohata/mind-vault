...
- **`AGENT_architect`** (`agents/AGENT_architect.md`) — 5-pass structural architecture workflow. Consumed by `skills/plan/` as a reviewer pass over draft plans.
...
- **Plan stage invokes `AGENT_architect` as reviewer, not author.** The skill drafts the plan; architect reviews it for coupling, scaling, abstraction. Rationale: architect's 5-pass workflow is a review shape, not a generation shape.
