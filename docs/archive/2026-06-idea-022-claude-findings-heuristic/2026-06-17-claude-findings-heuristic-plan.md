...
1. **Author the judge contract in `engine-claude.md`** (R5/R6/R7, architect S1) — judge prompt, tiered-verdict schema, the verbatim masking rule + proven-set fail-closed, the false-CLEAN-dangerous instruction, structural-vs-semantic split, fixture taxonomy. (Reference, not the always-loaded core.)
2. **Build the test layers FIRST** (Q2, architect C2/N1/N2): (a) deterministic material-surfacing test (`tests/test_claude_material_surfacing.sh`); (b) the **false-CLEAN hard-gate** fixtures — summary-body-only blocking finding, dual-verdict masking, marker-less-prose finding — asserting NOT CLEAN through the judge; (c) advisory CLEAN-vs-NON_BLOCKING eval. Run them against the *current* output first to confirm the fixtures are well-formed.
3. **Wire the dispatch into `skills/review-loop/SKILL.md` Phase 1** (thin): "for claude, run the judge per `engine-claude.md` → `{CLEAN|BLOCKING|NON_BLOCKING[]}`, cache per head SHA"; the typed carve-out on the `:67` "clean is structural" line; BLOCKING→iterate, CLEAN/NON_BLOCKING-only→clean-for-sync-gate; the R3 disposition (Tier-1 absorb / interactive-surface vs sprint-auto-auto-formalize via `SPRINT_AUTO_INTEGRATION_WORKTREE`); add the `claude_non_blocking[]` scratch slot (§ Scratch-file persistence). Reconcile `multi-engine-sync.md` scratch schema + convergence-gate composition.
4. **Now reduce `find_claude_comments.sh`** (R4) — only after steps 1-3 prove the replacement: REMOVE `CLAUDE_CLEAN_PATTERNS`/`CLAUDE_FINDING_MARKERS` (`:282-300`), `is_clean` (`:460`/`:521`), masking suppression (`:678-690`), summary-body finding render (`:820`); KEEP (named) STATUS aggregation (`:347-358`), in-window verdict enumeration + `WINDOW_START` (`:495-517`), `CLAUDE_VERDICT_SET_PROVEN` (`:551`), inline enumeration, raw summary body, `CLAUDE_NOOP_PATTERNS` (`:311`).
5. **Self-sweep** (`RULE_self-sweep-before-push`): shellcheck/pyflakes on the script, markdownlint on docs.
