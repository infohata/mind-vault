...
- **Verification recipe:** after the fix lands on the default branch (+ identical copy synced to the working branch), re-run on a PR carrying a **known critical bug** — if it now surfaces in a posted comment, the #1087 drop was the whole story. (This is what teisutis IDEA-214 exercises.)
