...
**Execution notes** (for `/plan`): see the emitted plan for the full phased sequence. Two themes phase cleanly: **Phase A** = wrap/land split + `/land` insertion + single-review chain on the *manual* path + `WRAP_BEFORE_REVIEW.md` rewrite; **Phase B** = sprint-auto S3–S7 two-pass → single-pass state-machine collapse (the larger, sprint-auto-only blast radius — and sprint-auto is already flagged UNSTABLE, so it phases behind the manual path). All under `RULE_rename-before-drop` sequencing (add `/land` → rewire → verify → drop legacy → re-verify).
