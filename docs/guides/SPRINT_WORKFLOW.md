...
- **Review stage is engine-agnostic.** Projects opt into `/review-loop <PR> bugbot` (Cursor Bugbot), `/review-loop <PR> copilot` (GitHub Copilot), `/review-loop <PR> claude` (Claude Code Review — the `claude-code-action@v1` + `code-review` plugin, push-triggered/comment-anchored), `/review-loop <PR> grok` (Grok Build — sticky CI review, push-triggered/comment-anchored), or any subset e.g. `/review-loop <PR> bugbot,copilot,claude,grok` — same phase structure, same hard bounds. `/compound` reads any engine's findings file as an input source and routes each cleared finding.
...
`/sprint-auto` is an optional wrapper around the **full sprint workflow** (stages 2–5) for unattended overnight execution on a VPS. Per IDEA it drives `/plan → /work → /wrap (idea-only, pre-merge) → review-loop (single pass over the wrapped PR) → escalation`; at batch end the integration phase merges + tears down, then it runs `/compound` per candidate and the review-loop on each mind-vault PR produced. The review-loop carries `bugbot`, `copilot`, `claude`, `grok`, or any subset per project config (`SPRINT_AUTO_REVIEW_ENGINE`), or is skipped when engine is `none` (curator-only fallback). T2/T3 review findings resolve autonomously via rollback-able fresh commits; per-pass attempt caps are 20 (per-IDEA review) / 5 (mind-vault compound), each an independent `/review-loop` session (budget spans all configured engines). The skill runs each IDEA in its own git worktree (code-surface only — no per-IDEA stack; a single shared integration docker stack at port offset `+30000` handles all verification), and stops at the HITL merge boundary per `RULE_git-safety`.
...
/review-loop <pr-url> <engine>    # 4b single review over the wrapped PR — bugbot | copilot | claude | grok | any subset
...
# ... fix ...
# open PR, /review-loop <PR> <engine> (bugbot, copilot, claude, or grok) clears it, maybe /compound if you learned something
```
