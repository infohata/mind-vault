...
The backstop's design constraints, learned building the git-safety one ([`../../hooks/block-protected-branch-writes.sh`](../../hooks/block-protected-branch-writes.sh)):
...
- **Enforcement lives with the rule, not the code under review.** Shipping the hook in `hooks/` next to the rule (and wiring it in `hooks.json`) means every consumer of the rule gets the enforcement automatically — the guarantee doesn't depend on each project remembering to add it.
