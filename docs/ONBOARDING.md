...
- **Sprint workflow** — a compounding 5-stage loop (`/ideate → /idea → /plan → /work → /bugbot-loop → /wrap → /compound`) where the final stage routes lessons back into mind-vault so the *next* sprint starts with a higher floor. See [SPRINT_WORKFLOW.md](SPRINT_WORKFLOW.md).
