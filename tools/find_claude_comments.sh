...
# ==============================================================================
# CALIBRATE (dogfood step 9 / Q1) — claude comment IDENTITY.
# ------------------------------------------------------------------------------
# The author login `anthropics/claude-code-action@v1` posts comments UNDER is
# UNCONFIRMED. On PR #166 (the install PR) the action ran but posted no review
# comments, so no live sample exists yet. Likely candidates (lock the exact one
# on the first review-bearing PR — `gh api .../pulls/<N>/comments --jq '.[].user.login'`):
#   - 'github-actions[bot]'        (the workflow token's default identity)
#   - 'claude[bot]' / 'claude-code-action[bot]'  (if the action posts under an app)
# A8: the filter is BOTH-AND — author login in the tuple AND the body carries the
# code-review plugin signature (CLAUDE_BODY_SIGNATURES). NEVER either alone, so an
# over-loose login (e.g. the shared 'github-actions[bot]') cannot alone mark a
# comment as claude's — the body signature disambiguates. Failing the BOTH-AND test
# means "not a claude comment" → no clean detected → keep polling (safe), never a
# false CLEAN.
#   CLAUDE_LOGINS (space-separated)            = 'github-actions[bot] claude[bot] claude-code-action[bot]'
# CALIBRATE (Q1) — the code-review plugin's comment-body SIGNATURE. The managed
# App stamps a recognizable header; the action+plugin's stamp is unconfirmed.
# Candidates to verify on the first run (case-insensitive substring, ANY-of):
#   'claude code review', 'generated by claude', '/code-review', 'reviewed by claude'
# Keep this CONSERVATIVE — a too-broad signature risks claiming a human's comment.
#   CLAUDE_BODY_SIGNATURES (regex alternation) = 'claude code review|generated by claude|reviewed by claude|code-review'
# CALIBRATE (A6) — the clean-body substring. Action issue #1087 + docs suggest
# "no issues found"; copilot burned TWO phrasings (find_copilot_comments.sh:182-189),
# so treat this as a single best-guess and lean on the zero-inline OR-arm until
# the live string is confirmed. Case-insensitive substring, not the full sentence.
#   CLAUDE_CLEAN_SUBSTRING                     = 'no issues found'
# ==============================================================================
# The three constants below are the SINGLE source of truth — exported once and
# read by every python pass via env. Update them HERE on calibration.
...
# ------------------------------------------------------------------------------
# Pass 2 — claude-authored head-SHA inline comments + summary comment (A8 filter)
# ------------------------------------------------------------------------------
# BOTH-AND identity (A8): login in CLAUDE_LOGINS AND body matches a signature.
# Inline findings are the staleness corpus; the summary comment (top-level issue
# comment) carries the clean substring. Emit:
...
def is_claude(c):
    # A8 BOTH-AND: author login AND body signature. Either alone is rejected
    # (fails toward no-clean → keep polling, never a false CLEAN).
...
if [ -n "$CLAUDE_CHECKRUN_LINE" ]; then
    ...
    SETTLE=${CLAUDE_REVIEW_SETTLE_SECONDS:-600}
    CLAUDE_DOWNGRADED=
