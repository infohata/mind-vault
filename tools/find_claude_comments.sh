...
# Finding markers are STRUCTURAL, not keyword-prose. A genuinely-clean review NAMES
...
# Classify the selected summary: clean (positive phrase AND no finding markers) vs
# findings-bearing. Both derived in one python pass for a single source of truth.
...
is_clean = 'true' if (has_clean_phrase and not has_finding_marker) else 'false'
# Findings-bearing: any finding marker present (regardless of a partial clean phrase).
has_findings = 'true' if has_finding_marker else 'false'
print(f'CLAUDE_SUMMARY_ID={cid} AT={at} CLEAN={is_clean} FINDINGS={has_findings}')
" 2>/dev/null || true)
