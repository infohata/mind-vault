...
- **Verify BOTH directions — a net-stack swap can silently kill EGRESS.** Changing the rootless net
  ...
  **active outbound probe from inside the container**, e.g.
  ...
  CA directory JSON (no `| head` — a pipe would mask `wget`'s exit code and the gate would fail open). Then confirm inbound: a request from a known external IP shows **that IP** in the
