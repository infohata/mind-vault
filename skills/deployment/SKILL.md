...
- [references/TRAEFIK_EDGE_HARDENING.md](references/TRAEFIK_EDGE_HARDENING.md) — public **Traefik v3** edge, no plugins: native **dotfile-404** (high-priority catch-all router + `ipAllowList`+`rejectStatusCode`, short-circuits before a dummy-server `noop` that must fail *closed*), the **`.well-known` carve-out** (RE2 has no lookahead → `&& !PathPrefix`; never block `/.well-known/acme-challenge/` or HTTP-01 renewal dies → cert expires → edge down), per-router rate-limit (omit `sourceCriterion`; `depth>0` trusts spoofable XFF), and the **version-bump audit drill** (latest-minor-only support, `acme.json` format unchanged v3.4–v3.7 → cert reused, `%2E` pre-routing normalization since v3.4.1, cert-reused-first gate + reviewed rollback). Pairs with ROOTLESS_DOCKER.md
