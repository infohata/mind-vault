...
Note what these four commands have in common: every one of them runs on the deploy host, against the tier that was just recreated. The next section is about the checks they structurally cannot make.

### Verify from the side that would experience the failure
...
**1 — The far end of a network path.** A probe run on the box it tests addresses one of that host's *own* addresses: the packet is delivered locally, never crosses the external path, and under the usual host-firewall default of a blanket accept on the loopback interface it simply connects. Such a probe proves the port is published and nothing more. It cannot tell "filtered" from "reachable", and it may pass for a reason unrelated to the control it names — a bind scope rather than a firewall rule — so when that unrelated reason later changes (a publish widening from one interface to all), the same fail-closed line starts *connecting* and fails the deploy for the opposite reason. Choosing the vantage is two decisions, not one:

- an assertion that something is **reachable** must be probed from an allow-listed consumer, as a named step with its own credentials and a delivery-path remedy in the failure message;
- an assertion that something is **blocked** must be probed from an origin that is *not* allow-listed for that path.

What decides which one you are actually running is the **source address the prober selects for that destination**, not which host you sit on — a multi-homed box picks a private source for a private destination and a public source for a public one, so the same machine sits inside the allow-list for one target and outside it for another. Keep the local probe, but *label* it the producer check. Then: grep the verify script for probe targets and check them against the host's own address list; re-derive every such assertion when the enforcement mechanism moves (bind-scoping → host firewall is a rewrite of the check, not a no-op); and sweep siblings, because the same silhouette usually sits in the neighbouring check on the box's other local address. A fail-closed assertion that has never once fired is a warning sign, not reassurance.

**2 — The tier the deploy did not recreate.** A deploy that recreates only the request-serving tier leaves workers, schedulers and CLI entrypoints on the **previous release** indefinitely, doing old work correctly — a skew health checks cannot see, because they probe the tier that was updated. The same asymmetry hits configuration: background tiers read values the request path never consults, so a malformed value can crash-loop the queue while the site serves perfectly. Neither failure emits an error at the web tier. The shape follows from the topology (request tier + background tier), not from any framework or orchestrator, and restates unchanged for containers, sidecar schedulers, systemd units and managed task services. So verify a deploy by asserting a fact **from** the background tier — the running worker's image or release identity, a value the scheduler reads back — not just the web tier's health endpoint; and for every config value, name which execution paths consume it. Expect pre-production not to reproduce this, often for an accidental reason nobody lists as a difference (a queue tier held down until a bundled database is seeded means nothing was ever running to go stale). Pairs with [references/MONITORING.md](references/MONITORING.md) § scheduler liveness — there the background tier is *dead* behind a green stack, here it is *alive on last release*.

**3 — The runtime the change was not tested in.** When one source tree is deployed to more than one runtime, a change to any file a second runtime also consumes is a production change to that runtime too, even when it is verified correct in the runtime that prompted it. The discriminator is **consumption**, not "repo-level-ness": a file no second runtime reads is not a fleet change. The propagation mechanism is the non-obvious half and is what makes it bite — a dependency manifest reaches an older host because deploys re-resolve dependencies (lockfile not committed); a framework config default bites because the untested runtime holds warm state (caches, existing directories) the tested one lacks; a shared template bites because its content hash triggers a fleet-wide regeneration sweep on the next deploy. The review question is therefore not "is this fix correct?" but *"which runtimes does this file reach, by what mechanism does the change take effect there, and what state do they hold that the tested runtime does not?"* Prefer scoping the change into an artifact only the intended runtime reads — a build stage, a per-instance env file, an override ([laravel / CONTAINER_BUILD_PLATFORM.md](../laravel/references/CONTAINER_BUILD_PLATFORM.md) is a worked example: pin the platform in the build stage, not in the shared manifest). Shipping it shared is legitimate, but only after proving it is a no-op on every other runtime and writing that proof into the commit body. This one is caught in review every time so far — the guardrail is what is holding, not luck.

Common failure of all three: choose detection primitives whose *absence* fails loudly. A CLI flag missing from an unpinned older version returns an empty set, reproducing the very false negative you were fixing.
...
- [references/CICD.md](references/CICD.md) — GitHub Actions, GitLab CI, secrets, approval gates, `paths-ignore` for image-building workflows (check the list against build inputs and required-check rules first)
- [references/MONITORING.md](references/MONITORING.md) — Prometheus, Grafana, ELK
- [references/ALERT_SILENT_DECAY.md](references/ALERT_SILENT_DECAY.md) — alerts that keep evaluating after they stop being able to detect their subject. A metric that changes **meaning** (not name) under a live rule reads the new world *wrongly* rather than not at all, so its silence still looks like good news — producer emits a NEW name for the new meaning; consumer greps rule files on every producer upgrade. A rule keyed on a **conditionally-emitted** series needs a coverage denominator, scoped to TOTAL absence — a per-subject guard fires for weeks on a correct estate and gets muted (`or vector(0)` on both sides is load-bearing: without it the guard against silence is itself silent). **Suppression windows** are pinned to a schedule someone else owns, so they degrade silently and in the NOISY direction — carry the measurement date, replay old-vs-new, and never suppress the outlier that is the only real signal in the sample. And **rule state is not delivery**: measure at the notifier, not the rule engine
- [references/DARK_DEPLOY_KILL_SWITCH.md](references/DARK_DEPLOY_KILL_SWITCH.md) — dark → shadow → live ladder; why a kill switch's OFF position must be faithful per call site; shadow OBSERVES but does not PROTECT (time in shadow has a running cost its own logs quantify); the staging unit is the ORGANIZATION, not the tenant; rollback stops future writes but cannot unwind past ones
- [references/DJANGO_DEPLOYMENT.md](references/DJANGO_DEPLOYMENT.md) — Django-specific optimizations (migrations, collectstatic, ASGI)
- [references/HARDENING.md](references/HARDENING.md) — SSH, UFW, fail2ban, unattended upgrades (run before first deploy); **fail2ban behind a proxy** — a filter that reads forwarded-IP log lines bans the end user while the proxy path stays open, and application-level 4xx must never count toward a flood; systemd unit-sandbox version gates (`SystemCallFilter=@system-service` needs ≥ 240 — older systemd resolves it silently and the loader dies `status=127`; gate drop-ins on the capability, never on virt-type)
...
- [references/CONFIG_STATES_INTENT.md](references/CONFIG_STATES_INTENT.md) — config states intent, only the running system states fact: five controls configured but inert on one host (TLS flag beaten by an `http://` scheme, audit refused by license tier, snapshot repo outside the path allow-list, firewall inactive, shipper authenticated as superuser); verify the effect never the setting, from outside and both sides; replace an inert setting with a why-absent comment; check every firewall backend before trusting an empty listing; an established TCP session can't be spoofed
