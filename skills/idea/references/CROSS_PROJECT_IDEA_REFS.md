...
Wired: idea SKILL.md § 4 (numbering independence — the attribution half lives here) · compound SKILL.md step 5 scrub table + optional grep aid · compound SKILL.md § 5 Cross-link (commit-message-only foreign refs) · compound SKILL.md auto-memory destination row + § THIS-MACHINE-ONLY routing test (memory notes) · rules/RULE_cross-idea-amendments.md (Amends-trailers are same-repo by construction).
