...
**When this fires**: post-merge `/wrap` runs, when the sprint ran in a parallel git worktree with its own docker-compose stack. Skip when running from the primary checkout, when the PR is still open (pre-merge), when the user has signalled keep-the-stack-up, or when the worktree has uncommitted work. The wrap SKILL.md body's Step 5 holds the firing-conditions stub; this reference holds the mechanics.
