...
- [MODERN_COMPONENT_FOOTGUNS](references/MODERN_COMPONENT_FOOTGUNS.md) — phantom ids, singleton+statics, empty-grid renderers, `emailfield`, native submit, checkbox `null`, deferred-callback teardown, wrapper-field binds, formulas as HTML, custom summaries, desktop/phone split, stay-open dialogs that must disarm Save (an unguarded one writes a duplicate per click), console ops against a live app (the first queried view is not the active one; destructive calls list-first and one at a time), invented `ui:` names that render invisible-but-functional chrome, a base `record` config that nulls any non-Model value, formulas an optional ancestor dependency blocks forever, desktop date pickers whose ui comes from `floatedPicker` over an invisible selected cell, string-template renderers that must encode at the sink in THREE contexts (html/attribute, JS-string-in-inline-handler, lossless DOM-id round-trip — one encoder covers only the first), the framework-level fix that belongs in `overrides/` once the second hand-copied guard is being written (`Ext.form.Panel` Enter guard with an opt-in `enterAction`), and the ViewModel-store silent-null family (a store whose config binds an UNDECLARED data key is never created at all — null forever, no error; declared ones are still null in the construction window, so manual `.load()` in a change handler needs a null guard; and a dialog open must never share a `.catch` with transport errors, or a render throw is misread as backend state).
...
- [PLAYWRIGHT_COMPONENTQUERY_E2E](references/PLAYWRIGHT_COMPONENTQUERY_E2E.md) — ComponentQuery bridge, single-handler mock bootstrap, `syncRequire`+`widget` mounting, toast reading, sentinel/`framenavigated` probe, dev-server collision + port reuse, phone-project skip, mock mode still fetching real external hosts (a dead one fails suites that never mention it) + the known-green-commit re-run that splits code from environment, boot probes on the production bundle (the boot chain handles its own rejection → console collector + failed-response collector, not `pageerror`; tolerated-404 whitelist with reasons; framework bare-`undefined` rejections tolerated by shape, traced with a Promise wrapper).
- [SENCHA_TOOLCHAIN_AND_BUILD](references/SENCHA_TOOLCHAIN_AND_BUILD.md) — JDK 8–11 Nashorn, portable JRE + `npm ci` order, workspace-lock hygiene, production-build gate, Docker image, cache policy, microloader double-launch, autobahn copy, registry token as BuildKit secret, swap-under-live-tab, the CI gate that builds the release image in Actions and runs the suite against it on both bundles (npm cripples Sencha Cmd — the Dockerfile is the one working toolchain; resolvable backend host, static e2e mode, never cancel a trunk run, docs-only `paths-ignore`), cross-origin dev login being dead in modern Chrome (`SameSite=Lax` + wildcard `Access-Control-Allow-Origin` with credentials → silent bounce-to-login; fix is a dev-server proxy mirroring production's nginx), one `sencha app watch` profile at a time with the microloader choosing the manifest by UA rather than by what the dev server built, and the post-deploy boot that runs the OLD cached bundle first behind a renderer-blocking native update confirm (verify a deploy against the server artifact, never a booted tab).
