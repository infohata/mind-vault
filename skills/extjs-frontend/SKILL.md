...
- [MODERN_COMPONENT_FOOTGUNS](references/MODERN_COMPONENT_FOOTGUNS.md) — phantom ids, singleton+statics, empty-grid renderers, `emailfield`, native submit, checkbox `null`, deferred-callback teardown, wrapper-field binds, formulas as HTML, custom summaries, desktop/phone split, stay-open dialogs that must disarm Save (an unguarded one writes a duplicate per click), console ops against a live app (the first queried view is not the active one; destructive calls list-first and one at a time), invented `ui:` names that render invisible-but-functional chrome, a base `record` config that nulls any non-Model value, formulas an optional ancestor dependency blocks forever, desktop date pickers whose ui comes from `floatedPicker` over an invisible selected cell, string-template renderers that must encode at the sink in THREE contexts (html/attribute, JS-string-in-inline-handler, lossless DOM-id round-trip — one encoder covers only the first), and the framework-level fix that belongs in `overrides/` once the second hand-copied guard is being written (`Ext.form.Panel` Enter guard with an opt-in `enterAction`).
