...
3. **Retrigger** — for each `<engine>` in `ENGINES` iterated **alphabetically** (deterministic order so behaviour is reproducible regardless of how the caller orders `ENGINES`, matching [`references/dual-engine-sync.md`](references/dual-engine-sync.md) § Retrigger discipline):
   - **New-push exemption (the common Phase 3 case): fire immediately, ignore the spacing window.** A Phase 3 retrigger always follows a fresh `git push` (step 2), and the loop only reaches Phase 3 after the prior SHA's review already returned the verdict that produced this fix cycle. A new SHA is a fresh review target with no pending same-SHA review to stack behind, so spacing does not apply — fire `./tools/<engine>_retrigger.sh [PR_NUMBER]` regardless of `last_<engine>_retrigger_at`, update it to the post-time, clear `pending_<engine>_retrigger`.
   - The 5-min spacing window below governs only **same-SHA re-triggers** — Phase 1 zero-activity that already fired once, or a manual re-poke — where a review for the current HEAD may still be in flight:
     - If `last_<engine>_retrigger_at` is unset OR ≥5 min ago: fire the retrigger. Update `last_<engine>_retrigger_at`. Clear `pending_<engine>_retrigger` if set.
     - Else (last retrigger <5 min ago, same SHA): do NOT fire now. Set `pending_<engine>_retrigger=true`. The defer for the slow engine does NOT block other engines firing this cycle — each engine is independent.
   ...
   **Spacing rationale**: engine queues stack — rapid same-engine retriggers **against the same SHA** don't preempt a pending review, they stack behind it. Field-observed: 4 bugbot retriggers in 10 min (no intervening commits) stretched per-review latency from ~1-10 min to ~16 min. The window only fires when there is a pending review on the current HEAD to stack behind; a retrigger on a brand-new push has nothing to stack behind, hence the exemption above. The rule is **per-engine** — under multi-engine mode firing bugbot + copilot back-to-back is fine (different queues); firing bugbot twice against one SHA in <5 min is not.
