...
- **Standing hand-back caveat**: when the loop hands back a converged PR whose engine set includes copilot, the hand-back carries one line — *"copilot CLEAN covers API-visible comments only; expand any suppressed comments in the PR UI before merging."* One human glance closes the gap the adapter cannot.
