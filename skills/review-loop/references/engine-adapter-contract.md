...
> **⚠️ CWD / repo-resolution gotcha — invoke from the TARGET PROJECT's checkout.** These scripts shell out to `gh`, which resolves the target repo from the **current directory's git remote** unless given `-R owner/repo`. When the scripts live in a *shared* tools dir (e.g. mind-vault is symlinked / cloned separately from the project under review), running them with the shared dir as CWD makes `gh` query the *wrong* repo — the PR number won't resolve, and `find_*` reports a false **"no activity yet"** while `*_retrigger` fails with `Could not resolve to a PullRequest`. Both are silent-ish: the false "no activity" is the dangerous one — the orchestrator may wrongly enter the zero-activity trigger branch or miss real findings. **Always invoke by absolute path while CWD is the project under review** (`cd <project> && /path/to/shared/tools/find_<engine>_comments.sh <PR>`), or have the script hardcode `gh -R`. When in doubt, cross-check with a direct `gh api repos/<owner>/<repo>/pulls/<PR>/reviews` — that's authoritative and CWD-independent, and is the recommended triage source when a find-script's output looks stale.
