...
[<ENGINE>_SUPPRESSED=<n> REVIEW=<review-id> COMMIT=<sha> AT=<iso8601>]   # AUTHORITATIVE (unlike CLEAN_SIGNAL): findings the engine withheld from the inline API, rendered only in the review body. Emit ONLY when n>0, followed by the items verbatim. They COUNT as active findings — n>0 for the head SHA ⇒ not clean
```
...
- `<ENGINE>_SUPPRESSED` is **authoritative and additive to the finding count**. An engine that withholds low-confidence findings from the inline-comment API but renders them in its review body (copilot's `### Suppressed comments (N)` block — [`engine-copilot.md`](engine-copilot.md) § Suppressed comments) MUST surface them under this marker with the items verbatim; the orchestrator adds them to the active-finding total for that verdict. Detecting such a block and merely withholding a clean signal is **not** sufficient: the orchestrator ignores the legacy `CLEAN=` token, so a detection that stops there is invisible to the verdict and ships real findings as CLEAN (the 2026-08-28 reversal in `engine-copilot.md`). Route the signal into the count, never into a second flag.
...
Engine-specific recurring findings the orchestrator can auto-fix without per-finding approval. The shared cross-engine catalog lives in [`common-review-findings.md`](common-review-findings.md); an adapter's § Common patterns section links there and documents only its engine-specific deltas.
