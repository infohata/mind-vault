...
- **Clean is WHOLE-REVIEW, not substring-anywhere.** claude clears sections independently: a single review says **"Bugs: No issues found."** in one section AND lists **"Docstrings missing"** in another. A bare `'no issues found' in body` test **false-cleans** that mixed review (the dangerous direction). Clean now requires a positive clean phrase (`no issues/bugs/problems found`) AND the **absence of any finding marker** (`missing|violation|❌|#### |### [0-9]` — structural, not security-keyword: a genuinely-clean review NAMES the concepts it checked, e.g. *"the privilege-escalation guard looks correct"*, so `privilege`/`escalation` as markers false-positive on clean prose). Verified: across all 13 downstream summary bodies the classifier gives 2 NOOP, 10 FINDINGS (incl. every mixed clean-bugs-but-dirty-docstrings review), 1 CLEAN (the final verdict) — zero false-cleans.
- **No-op skip bodies must be filtered.** claude posts `## Code review\n\nSkipped — …draft status` and `## Code review\n\nSkipped: …already reviewed this PR` issue comments that match the signature but are **not verdicts**. Keyed on the heading-then-`Skipped` shape (`code review\s+skipped`, catching em-dash / colon / hyphen variants) so a trailing skip-noop posted *after* a real verdict can't mask it, without dropping a real review whose prose merely mentions "skipped".
