...
| Claude stalled | Claude's Actions job `STATUS=in_progress` (RUNNING) past observed latency on `last_push_sha` — see [`engine-claude.md`](engine-claude.md) § Failure modes | Proceed with other engines' findings if any. **Do NOT explicitly retrigger** — claude is push-triggered, so the next Phase-3 fix push re-runs it implicitly (the `synchronize` auto-run). |
...
- For `claude`: **NO Phase-3 retrigger.** claude is push-triggered — the batch commit's push already fired the `synchronize` auto-run, so Phase 3 **skips** claude in the retrigger loop (firing `claude_retrigger.sh` here would double-run the action on one head SHA and create a "which run is authoritative" race). `claude_retrigger.sh` is reserved for the zero-activity bootstrap when no Actions run exists for the head SHA — see [`engine-claude.md`](engine-claude.md) § Push-triggered model.
...
The check-run/request-driven retriggers (bugbot, copilot) happen post-push and fire once each, back-to-back (different queues, no interval); claude's slot is a no-op (the push was its trigger). The orchestrator never retriggers while an engine's check-run/Actions job is RUNNING, so there is nothing to space out.
...
3. Slot the new engine into the **alphabetical** retrigger order — and if it's an **auto-trigger / comment-anchored** engine (push-triggered), note that Phase 3 SKIPS its retrigger (its `*_retrigger.sh` is bootstrap-fallback only), as `claude`'s slot documents.
