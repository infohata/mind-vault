...
`sprint-auto` runs N IDEAs in isolated git worktrees, each with its own docker stack. Each worktree's tests run **in isolation against that worktree's branch**. Bugbot reviews each branch **in isolation against `main`**.
...
- **Replacing per-PR bugbot.** The integration step is additive. Each `auto/*` PR still gets its own bugbot pass against `main`.
...
| Q6 (bugbot) | Fire on integration branch via `[INTEGRATION]` draft PR | user redirect |
...
| Bugbot coverage | Per-PR (deliverables + docs) AND integration AND post-propagation re-bugbot | user redirect (results-oriented = bugbot everything) |
| Env var | `SPRINT_AUTO_INTEGRATION_WORKTREE` | user redirect |
| Per-IDEA `.env` | Never created. Bugbot is review-only; verification routes to integration worktree | user redirect |
...
| Integration: merge + union + full suite + bugbot via [INTEGRATION] PR | ~60–120 min | Sequential merge ~5 min, union ~10 min, full suite ~25 min, bugbot avg ~30 min / worst-case ~80 min (cap 20 attempts — see S11.10) |
...
| **Total** | **~5–6.5 hours** (avg ~5 / elephant-batch worst case ~6.5) | Acceptable for overnight runs starting midnight–1 AM. Bugbot-loop's own 180-min active bound caps S11.10 worst case independently. |
...
2. **`/bugbot-loop`'s Phase 0**: skip entirely. No `.env` creation, no `docker compose up` in per-IDEA worktree. Bugbot is a code-review service; it doesn't need a runtime in the per-IDEA worktree. Fix-verification (the only step that needs runtime) routes to the integration worktree.
3. **Per-IDEA reset**: at the entry to S2 for each IDEA, sprint-auto runs `docker compose down -v && docker compose up -d && migrate + seed` in the integration worktree. This is the "main-equivalent baseline" for that IDEA's tests + bugbot session.
...
| S11.12 post-propagation re-bugbot | **5** | Small post-propagation surface; resolution + wrap commits on already-clean PR |
...
- `commands/bugbot-loop.md` — Phase 0 skip-bootstrap rule when env var set
...
- **2026-04-27 v3.1**: all open Qs resolved (env var name locked, bugbot per-IDEA worktrees no-`.env`/no-docker, per-IDEA worktrees not maintained as forensic artefacts, cap budgets accepted); stakes context added; ready for implementation
