...
5. **/wrap (pre-merge, idea-only scope) — S5.** Invoke `/wrap --scope=idea-only <NNN>` **before** the review (wrap-before-review: the single review then sees the PR at docs-finalized state) — narrows wrap to the IDEA-local subset:
...
6. **Review-loop the wrapped PR — single pass (S6).** Invoke `/review-loop <PR> $SPRINT_AUTO_REVIEW_ENGINE` (single call, all configured engines) **once**, over the wrapped PR — it reviews code AND finalized docs together (the loop's Phase-1 triage is finding-class-agnostic, so one pass absorbs both). This is the single-review cadence (IDEA-015): wrap-then-review, no separate deliverables/docs passes. With `SPRINT_AUTO_INTEGRATION_WORKTREE` set, the loop's Phase 0 skips entirely — per-IDEA worktrees never get `.env` or docker. Fix-verification routes to the integration worktree as in step 4 (no DB reset within the review session — fix commits don't typically migrate; reset cost would explode). Three outcomes (per [`references/escalation-policy.md`](references/escalation-policy.md)):
