...
- **Bugbot-loop invocations are MANDATORY; ship-non-clean is a fallback, not an upfront opt-out.** The skill's S3 and S6 contracts are "invoke `/bugbot-loop`, work the escalation cycle, ship-non-clean only after the per-pass cap is reached." They are NOT "decide upfront not to invoke and call it ship-non-clean." Reasoning the skill must accept (e.g. "overnight harness can't reliably wait for async bugbot polls", "would exceed the 5-hour wall-clock budget", "Cursor Bugbot is async") is **not a license to skip the attempt** — bugbot-loop is itself bounded (180 min active, 20 idle polls, 20 commits) precisely so it can be fired-and-forgotten under sprint-auto, with the loop's own bounds enforcing the time ceiling. If the budget *runs out* mid-loop the loop hands back its findings as Tier-3 escalations and sprint-auto continues — which is the *intended* path to a non-clean ship. Captured 2026-04-26 from teisutis batch (122/124/125/126/128/129) where the agent shipped six PRs without firing `/bugbot-loop` on any, justifying it as "ship-non-clean is OK." User correction: shipping non-clean is OK when you cannot manage; **skipping the attempt without trying is NOT OK**. The auto-run log must record the attempt outcome (clean / unresolved / budget-exceeded) for each pass, NOT `deferred to morning review` — the latter is a tell that no attempt was made.
...
- **Non-clean bugbot is OK to ship-for-review — *after* the attempt was made.** After the per-pass cap is hit (20 on deliverables, 5 on docs, 5 on mind-vault compound PRs) OR `/bugbot-loop` returned an exhaust-budget signal, an IDEA with unresolved Tier 2/3 findings still produces a valid PR. The morning reviewer decides whether the remaining findings block merge. Do not block the pipeline on perfection; transparency in the auto-run log (attempt SHAs, approaches, outcomes) is the contract. **The corollary is the rule above:** "non-clean is OK" is a release valve for genuine cap-hit / budget-exhausted cases, not a justification for skipping `/bugbot-loop` invocation entirely.
...
**Last Updated**: 2026-04-26 (tightened the bugbot-loop rule pair to close a gap surfaced by the teisutis 2026-04-26 overnight batch: the agent shipped six PRs without firing `/bugbot-loop` on any of them, citing "ship-non-clean is OK." Added the explicit `Bugbot-loop invocations are MANDATORY; ship-non-clean is a fallback, not an upfront opt-out` interaction rule, with an anti-pattern callout naming common skip-the-attempt rationales ("overnight harness can't wait for async polls", "would exceed wall-clock budget", "Cursor Bugbot is async") that aren't valid licenses. Tightened the existing `Non-clean is OK to ship-for-review` rule to refer back as its corollary — non-clean is a release valve for real cap-hit / budget-exhausted cases, not a justification for skipping the invocation. Auto-run log signal: `deferred to morning review` is now a tell that no attempt was made and the rule was violated.)

**Previous**: 2026-04-22 (structural reconciliation pass: S0–S11 per-IDEA + S12–S15 batch state machine, canonical failure-path invariant (re-enter at step 8, harvest + log always run), two-pass bugbot-loop inserted — deliverables pass after /work, docs pass after /wrap (pre-merge). Escalation caps bumped from a placeholder 3 to calibrated per-pass numbers: 20 deliverables / 5 docs / 5 mind-vault compound, each independent — reflects real tail-length of T2/T3 resolution work.)
