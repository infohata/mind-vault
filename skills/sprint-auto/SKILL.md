...
- **Two-pass review-loop per IDEA + integration review.** Every successful-through-PR IDEA run receives **two** per-PR review-loop invocations (deliverables S3, docs S6); the batch gets one integration-state review (S11.10). Each is one `/review-loop` session with its own independent escalation budget — 20 deliverables / 5 docs per IDEA, plus 20 integration. When `SPRINT_AUTO_REVIEW_ENGINE` lists multiple engines, the single multi-engine session shares that budget across engines (per the budget rule below) — "20 deliverables" is 20 commit-cycles for the session, not 20-per-engine. Skipping any of these passes is never correct on the happy path. (v3.2 deletes v3.1's per-PR re-review S11.12 — the per-IDEA PR's head doesn't change after S6, so re-running adds no signal.)
...
- **Budget is the `/review-loop` session cap, not per-engine.** A multi-engine `/review-loop` runs ONE session whose caps (`max_commits_per_session=20`, `max_idle_polls=20`) span all engines, batching every engine's findings into one fix commit per cycle. So "20 deliverables" = 20 commit-cycles for the deliverables session regardless of how many engines are configured — not 20-per-engine. Per-engine clearance is still tracked inside the loop (asymmetric-clearance hand-back); the *budget* is shared.
