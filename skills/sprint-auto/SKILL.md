...
- **Forward-sync makes per-PR PRs co-dependent — flag it loudly to the morning reviewer.** After S11.11, every per-PR PR carries the entire batch's content. The first PR merged absorbs all batch IDEAs; subsequent PRs' diffs collapse to zero against main. Pre-first-merge, all PRs in the batch show similarly-large GitHub diffs (computed against pre-batch main). This is the documented design — atomic-batch merging is the win — but it surprises a reviewer expecting per-PR review independence. **The auto-run summary's morning checklist MUST carry the explicit warning** (see `assets/auto-run-log-template.md` § "⚠️ Co-dependent PRs"); per-PR PR descriptions SHOULD carry a co-dependent-batch banner injected at S11.11 push time. Without those notifications, the reviewer wastes time hunting for "what's unique to PR-B" — the answer is "nothing, by design." See `references/integration-stage.md` § Forward-sync for the full trade-off discussion. *(Compounded 2026-04-29 from teisutis batch where the reviewer merged PR #397 successfully then asked "PR #398's diff looked weird" — exactly because it had collapsed to zero. The mechanics worked correctly; the morning notification did not.)*
...
**Last Updated**: 2026-04-29 (interaction rule + 3-place doc update: forward-sync (S11.11) makes per-PR PRs co-dependent — first merge absorbs the whole batch; subsequent PRs' diffs collapse to zero. New rule + summary template "⚠️ Co-dependent PRs" warning + strengthened integration-stage.md trade-off section. Compounded from teisutis 2026-04-29 batch where the morning reviewer merged PR #397 cleanly but flagged "PR #398's diff looked weird" — exactly because it had collapsed to zero. The mechanics worked correctly; the morning notification did not.)
