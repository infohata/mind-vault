...
5. **Review-loop the PR — deliverables pass (S3).** Invoke `/review-loop <PR> $SPRINT_AUTO_REVIEW_ENGINE` (single call, all configured engines) on the PR `/work` just opened. With `SPRINT_AUTO_INTEGRATION_WORKTREE` set, the loop's Phase 0 skips entirely — per-IDEA worktrees never get `.env` or docker. Fix-verification routes to the integration worktree as in step 4 (no DB reset within the review session — fix commits don't typically migrate; reset cost would explode). Three outcomes (per [`references/escalation-policy.md`](references/escalation-policy.md)):
   - **Clean** → step 6.
...
7. **Review-loop the PR — docs pass (S6).** Invoke `/review-loop <PR> $SPRINT_AUTO_REVIEW_ENGINE` again, on the PR with the docs commit(s) on top. Same Phase-0-skip rule. Three outcomes as in step 5.
...
- **Rollback-able commits only for escalation resolution.** During step-5a, step-7a, S11.8/S11.9 fix attempts, and S11.10 review escalation, the skill MAY commit multiple attempts, each revertable via `git revert`. It MAY NOT force-push, rewrite history, or amend commits produced by `/work` or by `/wrap`'s docs sweep — those are part of the PR's review history. If a fix attempt goes wrong, the correct move is `git revert <bad-sha>` + a fresh attempt commit.
- **Two-pass review-loop per IDEA + integration review.** Every successful-through-PR IDEA run receives **two** per-PR review-loop invocations (deliverables S3, docs S6); the batch gets one integration-state review (S11.10). Each pass has its own independent **escalation-attempt** budget — 20 deliverables / 5 docs per IDEA, plus 20 integration (sprint-auto re-entries to resolve T2/T3 findings; see the budget rule below). When `SPRINT_AUTO_REVIEW_ENGINE` lists multiple engines, one multi-engine session runs per pass, so that budget is shared across engines — "20 deliverables" is 20 escalation attempts for the single deliverables pass, not 20-per-engine (which two sequential single-engine loops would have given). Skipping any of these passes is never correct on the happy path. (v3.2 deletes v3.1's per-PR re-review S11.12 — the per-IDEA PR's head doesn't change after S6, so re-running adds no signal.)
- **Review-engine selector — project-configurable, default `none`. One `/review-loop` call carries all configured engines.** Sprint-auto's S3 / S6 / S11.10 / S13 / S14 review passes each dispatch a **single** `/review-loop <PR> $SPRINT_AUTO_REVIEW_ENGINE` invocation, passing the whole resolved engine list. `SPRINT_AUTO_REVIEW_ENGINE` is exported during preflight from the project's `review_engine` declaration in `CLAUDE.md` or `.mind-vault.yml`:
    ...
    - `SPRINT_AUTO_REVIEW_ENGINE=bugbot,copilot` (or list form `review_engine: [bugbot, copilot]` in YAML) → `/review-loop <PR> bugbot,copilot` — review-loop's **concurrent multi-engine sync** (wait for the slowest engine per push SHA, batch all engines' findings into one fix commit, retrigger all). A "clean" verdict requires every engine to clear. Generalises to N engines with no change here.
...
- **Escalation budget is shared across engines, not multiplied — and is distinct from the loop's internal session bounds.** Sprint-auto's per-pass caps (20 deliverables / 5 docs / 10 union / 10 full / 20 integration / 5 compound) count **escalation attempts** — sprint-auto re-entries to resolve T2/T3 findings, tallied as `deliverables_escalation_attempts` / `docs_escalation_attempts` (see [`references/escalation-policy.md`](references/escalation-policy.md)). With one multi-engine `/review-loop` session per pass (vs the old two sequential single-engine loops), that cap is a single shared budget, NOT 20-per-engine. This is **separate** from `/review-loop`'s own internal `max_commits_per_session` / `max_idle_polls` bounds, which govern fix commits *within* a single session, not sprint-auto's escalation re-entries. Per-engine clearance is tracked inside the loop (asymmetric-clearance hand-back); the escalation budget is shared.
- **Review-loop invocations are MANDATORY (when an engine is configured); ship-non-clean is a fallback, not an upfront opt-out.** When `SPRINT_AUTO_REVIEW_ENGINE` is not `none`, the skill's S3, S6, and S11.10 contracts are "invoke `/review-loop <PR> $SPRINT_AUTO_REVIEW_ENGINE`, work the escalation cycle, ship-non-clean only after the session cap is reached." They are NOT "decide upfront not to invoke and call it ship-non-clean." The auto-run log must record the attempt outcome per pass (per-engine verdicts within), NOT `deferred to morning review` — the latter is a tell that no attempt was made. (When `SPRINT_AUTO_REVIEW_ENGINE=none`, the pass is logged as `review_pass: skipped (engine=none)` per the selector rule.)
...
- [references/escalation-policy.md](references/escalation-policy.md) — T2/T3 resolution rules + per-pass attempt caps (20 deliverables / 5 docs / 10 union / 10 full / 20 integration / 5 mind-vault compound)
