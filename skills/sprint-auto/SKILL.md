...
- **Eval-gate is the unblock for UX-overhaul / a11y / interaction-heavy IDEAs.** When an IDEA's *implementation* is mechanical (component scaffolding + JS API + render-and-assert tests) but ships behavior that needs human eyes on visual / focus / screen-reader / animation residue, opt-in via `auto_safe_with_eval_gate: true` instead of leaving the IDEA out of sprint-auto entirely. Sprint-auto runs the IDEA end-to-end without pausing; `/wrap` S5 emits a structured checklist to the IDEA's archive dir (see [`../wrap/SKILL.md`](../wrap/SKILL.md) § Step 7); S11.10 aggregates every emitted checklist's URL into the integration PR's body. The morning reviewer's existing PR-merge gate is now also the manual-walk gate — same review surface, structured in.
...
- [references/integration-conflict-resolutions.md](references/integration-conflict-resolutions.md) — **NEW** algorithm catalog for S11.6 conflict resolution (devlog, index, .po, HTML, JS, Python, settings, tests)
