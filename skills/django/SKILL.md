...
- [Form-invalid status](references/FORM_INVALID_STATUS.md) — Django's default `form_invalid` returns 200 + form-with-errors, NOT 422; status-only gating closes HTMX modals on validation failure; fix via `HTMXFormStatusMixin` or `HX-Trigger` header gate. **Paired client-side requirement**: HTMX won't swap 422 by default — needs a project-wide `htmx:beforeSwap` listener that marks 422 as swap-eligible + non-error, otherwise the form body never lands and the toast fires instead. Both halves wired together; either missing produces silent-fail.
- [ModelForm `_post_clean` trap — snapshot fields BEFORE bind](references/MODELFORM_POST_CLEAN_TRAP.md) — Django's `ModelForm.is_valid()` calls `_post_clean()` which writes `cleaned_data` INTO `self.instance` via `construct_instance`. Reading `instance.field` AFTER `is_valid()` always returns the POST value, defeating any change-detection logic written around the bound instance. `form.changed_data` doesn't help either (initial may auto-populate via smart defaults). Fix: snapshot the field's saved value BEFORE binding the form. Bites locale switching, audit logs, cache invalidation, side-effect emails, webhook dispatch.
