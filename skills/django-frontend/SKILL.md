...
- [Preview Drawer URL Stack Contract](references/PREVIEW_DRAWER_URL_STACK.md) — `?open=<base>&push=<f1>,<f2>,…` for round-tripping a megastack-style preview drawer's full state through the URL; 7-phase migration shape + popstate-without-state failure mode + URL-pattern-map fallback; state-mutation patterns: `store.top` is a snapshot getter / walker rebind dispatches all three HTMX events / edit-frame guard / universal edit→detail invariant in walker / empty-snapshot fallback on pop / `data-preview-route="open"` for back-navigation / `openWith(prefixFrames, topFrame)` LCP-dedupe for natural-parent stacking via server-emitted `data-preview-stack-prefix`
...
- [Collapsible Patterns](references/COLLAPSIBLE_PATTERNS.md) — native `<details>` defragilization (chevron / lazy-fetch / persist via `toggle` event + sessionStorage; eliminates Alpine `open`/`loaded`/`x-show`/`x-cloak`/`x-transition`/`x-effect` desync); lazy-load bucket recipe (eager priority bucket + lazy rest via `:lazy_fetch_url`, counts upfront, render-fn-sibling for three consumers)
...
- [Preview Drawer URL Stack](references/PREVIEW_DRAWER_URL_STACK.md) — `?open=&push=` URL contract + state-mutation patterns (`store.top` snapshot trap, walker rebind, edit-frame guard, universal edit→detail invariant, empty-snapshot fallback, `data-preview-route="open"`, `openWith()` LCP-dedupe natural-parent stacking)
...
- [Collapsible Patterns](references/COLLAPSIBLE_PATTERNS.md) — native `<details>` + `toggle` event for chevron/lazy-fetch/persist (eliminates Alpine state-machine desync); lazy-load bucket recipe for multi-bucket surfaces with cheap counts upfront
