...
- [Preview Drawer URL Stack](references/PREVIEW_DRAWER_URL_STACK.md) — `?open=&push=` URL contract + state-mutation patterns (`store.top` snapshot trap, walker rebind, edit-frame guard, universal edit→detail invariant, empty-snapshot fallback, `data-preview-route="open"`)
