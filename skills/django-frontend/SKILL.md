...
- [Diagnostic instrumentation hygiene — `_trace()` arg-eval trap](references/DIAGNOSTIC_INSTRUMENTATION_HYGIENE.md) — flag-gated JS diagnostics (`window.X_TRACE`) ship a lazy `_traceFn(label, () => payload)` variant from the start; JS evaluates args before the wrapper's flag check, so caller-supplied `querySelectorAll` / `dataset` / `closest` payloads run on every call even when the flag is off; gate at handler-call-sites, cache values used by both code paths and trace payload, never compute twice. Both Bugbot and Copilot flag the eager-payload shape reliably
