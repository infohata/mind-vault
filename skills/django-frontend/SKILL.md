...
- [Form rendering patterns — Bulma chrome, three valid shapes + one trap](references/FORM_RENDERING_PATTERNS.md) — `{% crispy form %}` (preferred) / manual `<input class="input">` markup (custom layouts) / bare `{{ form.field }}` with widget-attrs (compromise) are the three valid shapes. The trap is bare `{{ form.field }}` WITHOUT widget classes — produces unstyled native HTML inside otherwise-Bulma-shaped wrappers. Plus the Django help_text `<p>` vs `<div>` wrapper trap when help_text contains block-level HTML (PasswordChangeForm `<ul>` escape case). Shape-selection table + testing mechanics.
