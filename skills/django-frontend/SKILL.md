...
- [SCSS responsive patterns](references/SCSS_RESPONSIVE_PATTERNS.md) — shared styling that differs at a breakpoint: (1) `@extend` can't cross `@media` → use a `@mixin` (+ the compiler-path-masking trap — verify on the strict e2e/CI compiler, not just the permissive dev build); (2) cascading CSS custom property on the root as one source for a responsive footprint consumed by ≥2 components; (3) additive-padding collapse — designate one gutter owner, zero nested layers' horizontal padding at the constrained breakpoint
