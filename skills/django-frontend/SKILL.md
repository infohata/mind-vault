...
- [Visual-acuity tests via Playwright](references/VISUAL_ACUITY_TESTS_VIA_PLAYWRIGHT.md) — when to write Playwright vs render-and-assert; first-suite cost-benefit threshold (≥3 documented regression classes); docker bootstrap mechanics learned the hard way (separate requirements-e2e.txt; MS upstream image's Python version gap; `audioop-lts`-style transitive trap; chromium refuses Host overrides; ALLOWED_HOSTS + non-primary Domain row routing; `django_db_setup` unblock + `DJANGO_ALLOW_ASYNC_UNSAFE` for live-DB e2e; don't volume-mount a file under a directory mount)
