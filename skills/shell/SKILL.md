...
- [references/CREDENTIALS_IN_OPERATOR_COMMANDS.md](references/CREDENTIALS_IN_OPERATOR_COMMANDS.md) — a command handed to a human that reads a config file prints only the fields it needs, or redacts by key name AND value shape (a grep for the service name matched the service-named token line; a keyword list alone misses tokens under plain keys); request bodies carrying secrets go over stdin, not argv (process table), and the argv-logging test that proves it; least privilege vs your own verification — count via a stats/metadata endpoint, not a document read, and validate the new credential before editing config.
