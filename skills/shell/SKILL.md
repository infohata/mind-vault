...
- [references/INTERACTIVE_SUDO_LOGIN_SHELL.md](references/INTERACTIVE_SUDO_LOGIN_SHELL.md) — the *interactive* sibling of the above: `sudo -iu <user>` / `sudo -i` / `su -` opens a login shell that blocks until it exits, so `&&`/`;`-chained commands run in the PRE-sudo session, not the target user's (and a pasted multi-line block feeds its tail to the caller's shell). Fix: put the become-user command on its own line (everything after runs *inside* that session), or one-shot `sudo -iu <user> bash -lc '…'` (`-l` sources the login env — PATH, `XDG_RUNTIME_DIR`, rootless `DOCKER_HOST`). Common when writing rootless-docker service-user runbooks.
