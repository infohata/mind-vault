...
- [references/PRIVILEGE_DROP_PORTABILITY.md](references/PRIVILEGE_DROP_PORTABILITY.md) — `runuser` is not on Debian; use `setpriv --reuid … -- env HOME=… <cmd>` (argv-preserving) or `su -s /bin/bash <user> -c` (shell-string, mind the quoting/password-context); add the chosen tool to the dep preflight.
