...
validate untrusted values with `case`, not `grep -E` (newline injection); never end a Python triple-quoted literal that writes shell on its own quote character;
...
- [references/QUOTING_AND_INPUT_HYGIENE.md](references/QUOTING_AND_INPUT_HYGIENE.md) — quoting/arrays/`"$@"`, `printf` over `echo`, `IFS= read -r` + subshell trap, `case`-not-`grep` validation, arg parsing (`$2` guard, getopts vs manual, no `getopt(1)`), heredoc quoting, anchored status greps, generating shell from a Python triple-quoted string (a content-final quote is swallowed by the delimiter).
...
- [references/MAINTENANCE_SCRIPT_CONTRACT.md](references/MAINTENANCE_SCRIPT_CONTRACT.md) — mode surface (DRY-RUN / `--apply` / `--verify` / `--revert` / `--check`), the interactive precondition-acknowledgement gate, one-target-at-a-time, idempotency, evidence logs, lockout discipline, grep portability, the runnable-recovery-recipe contract (classify-then-advise or refuse; parse porcelain by columns, not whitespace), the emitted-artifact traps (staleness stamp, foreign-owned `/tmp` path), and preconditions that assert a PROPERTY probed from the consuming context — never a forbidden NAME encoding one topology (answer bad / good / can't-tell; can't-tell warns, it doesn't block).
- [references/VERIFY_DISCIPLINE.md](references/VERIFY_DISCIPLINE.md) — the craft of the `--verify` half, split out of the contract: assert the POSITIVE code so an unreachable remote target fails closed (plus rate-limit load tests need concurrency; `openssl x509` has no `-notBefore`), test the operation not its guard (break the TARGET, not the input), error paths under the same contract as the happy path, verify the exact file you edited never a glob, a zero is only evidence when the method can produce a hit, and prove a log line lands before a decision rests on an empty log.
- [references/EVIDENCE_SCRIPTS_AND_FALSE_CLEANS.md](references/EVIDENCE_SCRIPTS_AND_FALSE_CLEANS.md) — read-only scripts whose entire product is a claim ("nobody hit the fault"): the check that reports success without having run, sweeping the CLASS across discovery/filter/parse/aggregate/print rather than patching the instance, printing the positive count beside the zero, naming the distinct reasons for an empty result, announcing partial coverage (a missing section reads as a complete report), and deriving targets from the CONFIG rather than a directory / unit-name / default-path guess, and the extraction that is wrong while the data is right (a `uniq -f` that compares nothing, a validator whose diagnostic is discarded or capped away, a substring filter that breaks only once the awaited event happens). Plus the passing-is-the-default-state-of-a-broken-check family: the assertion that COULD NOT run scoring as passed (three-state ASSERTED / COULD-NOT-RUN / FAILED; the `--apply && --verify` list trap), fail-open branches naming an EXTERNAL cause for an internal defect, where the unknown lands (fillers cannot represent "unset"), and threshold calibration with the verdict's own expression.
- [references/GREEN_RUN_UNIVERSE_TOO_SMALL.md](references/GREEN_RUN_UNIVERSE_TOO_SMALL.md) — the neighbour of a false clean: the check ran honestly but its universe could not express the defect. Forgiving substitutes (empty state, weaker store, un-taken failure branch), breaking the TARGET to enter an error path, environment divergence (PATH, tty, login shell), a probe that bypasses part of the production path (and alerts on downstream consequences / thresholds not derived from the process they back up), and writing the bound beside the verdict.
- [references/DELETING_SOMEONE_ELSES_DATA.md](references/DELETING_SOMEONE_ELSES_DATA.md) — destructive work authorized by a message, not a spec: pin every target by path + size + mtime from the approved survey, a literal list never a glob, verify all targets and keepers before unlinking any, assert survivors, stay narrower than the approval, test the refusals.
- [references/CREDENTIALS_IN_OPERATOR_COMMANDS.md](references/CREDENTIALS_IN_OPERATOR_COMMANDS.md) — a command handed to a human that reads a config file prints only the fields it needs, or shows keys with every value hidden except allow-listed safe keys (a grep for the service name matched the service-named token line; a secret *scanner* always has a bypass, so invert it); request bodies carrying secrets go over stdin, not argv (process table), and the argv-logging test that proves it; least privilege vs your own verification — count via a stats/metadata endpoint, not a document read, and validate the new credential before editing config.
