...
2. **Answer all three states** — *bad*, *good*, and *can't tell*. A probe whose own
   ...
   always answer. It is not a license to downgrade a guard that *can* answer.
...
Deliberate tension with the verify-side rule in
[`VERIFY_DISCIPLINE.md`](VERIFY_DISCIPLINE.md) § Remote black-box `--verify`, and it is not a
contradiction:
...
The craft of the `--verify` half — assert the positive code so an unreachable target fails
closed, break the TARGET not the input, error paths under the same contract, verify the exact
file you edited, a zero only counts when the method can produce a hit, prove a log line lands —
lives in [`VERIFY_DISCIPLINE.md`](VERIFY_DISCIPLINE.md).
