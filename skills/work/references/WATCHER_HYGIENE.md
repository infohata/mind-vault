...
7. **Durable state keyed to an external subject is retired when that subject closes — by the writer, at its own terminal step.** A watcher's task output is ephemeral and bounded (Hard Rule 6); *scratch state* is neither. A loop that checkpoints per-PR state to a durable path so it survives compaction has written a file whose subject has a **defined end** — the PR merges — and nothing at that moment deletes it. The write-site fires every cycle; no site fires on close. Measured: one project's review-loop scratch dir held **41 files spanning three months**, every one keyed to an already-merged PR, and 108 across all projects on the machine — accumulated the whole time Rule 6 existed, because that rule is *optional*, addressed to nobody in particular, and scoped to `tasks/`.

   The obligation belongs to the **writer, at its terminal branch**: whatever step declares the loop finished — a terminal hand-back, a CLEAN verdict, a closed subject — deletes the state file it has been maintaining. Not a session-end sweep and not a periodic cleanup somebody remembers; the same code path that says "done" says it in the filesystem too. **Confirm the subject is actually closed before deleting** — an *open* subject's file is live resumable state, not garbage, and any sweep that ages files out by mtime cannot tell those two apart.
