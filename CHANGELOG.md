...
- **`skills/shell/references/CREDENTIALS_IN_OPERATOR_COMMANDS.md`** (new) — a command handed to a human that reads a config file must not be able to print a secret: a grep for the service's name matched the line holding the service's token, and it went into a chat. Prefer printing only the fields you need (scrubbing `user:password@` from URLs, since an allowed field can still carry one); when output must be broader, show every key but hide every value except under keys named as safe, because any secret scanner has a bypass (a token under a plain key, a JWT with dots). Secrets in request bodies go over stdin at every step, not on any command line, where every user can read them in the process table (building the body with `jq --arg` leaks it just as `curl -d` does). And least privilege can break your own verification: count documents through a metadata endpoint that a write-only account is allowed to call.
