...
- **`skills/shell/references/STRICT_MODE_HAZARDS.md` § 15** — in a shell a human is sitting in (interactive `sudo -i` / `su -` / `ssh`), the script idioms `exit 1` in a guard and top-level `set -e` close the operator's session instead of ending a script, so a deliberate refusal becomes a session kill and the evidence goes with it. In the incident it was the `exit 1` that killed the session (the `||` exempts the test from `set -e`). The fix is to run the block in a `( set -e … )` subshell, which refuses visibly and keeps the session.
