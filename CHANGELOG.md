...
- **`skills/deployment/references/CONFIG_STATES_INTENT.md`** (new) — verify a control by its effect, never by its setting: "documents rose by 15 in ten seconds" is a check, "the service started" is not, and a growing audit file is not audit events. Verify from outside the host and on both sides (blocked path blocked, allowed path still working). Replace a setting that does nothing with a comment saying why the control is absent, rather than deleting it quietly. When a firewall listing is empty, check the other backend: a forgotten one-address block there hid an exposure for years. Also: an address in an established TCP session can't be spoofed, though it may be a NAT, proxy or load balancer rather than the real client.
- **`skills/shell/references/CREDENTIALS_IN_OPERATOR_COMMANDS.md`** (new) — a command handed to a human that reads a config file must not be able to print a secret: a grep for the service's name matched the line holding the service's token, and it went into a chat. Prefer printing only the fields you need, cutting any URL down to `scheme://host:port` and dropping inline comments (an allowed field can still carry a token in a URL's userinfo, path or query); when output must be broader, show every key but hide every value except under keys named as safe, because any secret scanner has a bypass (a token under a plain key, a JWT with dots). Secrets in request bodies go over stdin at every step, not on any command line, where every user can read them in the process table (building the body with `jq --arg` leaks it just as `curl -d` does). And least privilege can break your own verification: count documents through a metadata endpoint that a write-only account is allowed to call.
...
- **`skills/shell/references/MAINTENANCE_SCRIPT_CONTRACT.md` split** (553 → 389 lines, back under the 500-line guideline) — its last eight sections, all about proving a change worked, moved into a new **`skills/shell/references/VERIFY_DISCIPLINE.md`**: assert the positive status code, break the target not the input, error paths, verify the exact file you edited, a zero only counts if the method can find a hit, prove a log line lands. The move itself was byte-identical; review of the new file then hardened four of its examples: the rate-limit load test now requires `RATELIMIT_PROBE=1`, passes the URL as data (a `$` in a query string could otherwise run as a command) and fails when nothing is throttled; the zero-count `grep -c` no longer ends a `set -e` script before its positive control; the restore helper uses a `mktemp` file instead of a predictable `$$` name; and the log smoke test no longer wipes an existing log and now greps for a marker unique to its own run, failing with exit 1 when it is absent. The contract ends with a pointer to the new file; the shell SKILL.md References and the Traefik reference link both files.
...
- **`skills/shell/references/STRICT_MODE_HAZARDS.md` § 15** — in a shell a human is sitting in (interactive `sudo -i` / `su -` / `ssh`), the script idioms `exit 1` in a guard and top-level `set -e` close the operator's session instead of ending a script, so a deliberate refusal becomes a session kill and the evidence goes with it. In the incident it was the `exit 1` that killed the session (the `||` exempts the test from `set -e`). The fix is to run the block in a `( set -eo pipefail … )` subshell after `set +e`, capturing its status with `; rc=$?` rather than `||` (which would disable `set -e` inside it), so it refuses visibly and keeps the session.
