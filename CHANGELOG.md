...
- **`skills/shell/references/CREDENTIALS_IN_OPERATOR_COMMANDS.md`** (new) — a command handed to a human that reads a config file must not be able to print a secret: a grep for the service's name matched the line holding the service's token, and it went into a chat. Prefer printing only the fields you need; when output must be broader, redact by key name and by value shape, since a keyword list misses a token stored under a plain key. Secrets in request bodies go over stdin at every step, not on any command line, where every user can read them in the process table (building the body with `jq --arg` leaks it just as `curl -d` does). And least privilege can break your own verification: count documents through a metadata endpoint that a write-only account is allowed to call.
