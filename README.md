...
| **extjs-frontend**     | Sencha ExtJS 7 Modern SPA conventions across the 4 contract concerns — ViewModel/ViewController MVVM (formulas, binds, the Array-not-Collection / phantom-id / `findRecord` traps), JSON-envelope API with a promise service layer over `Ext.Ajax` (no fragments), `Ext.define`+`ui`-catalogue component system with a shared/desktop/phone split, loadmask + native-submit Enter guard as the form lock; references for the Jest Ext-stub harness, Playwright ComponentQuery e2e, i18n key sweep, Sencha Cmd/webpack toolchain + build gate. |
