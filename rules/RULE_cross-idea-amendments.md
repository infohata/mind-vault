...
1. **Tag the commit message** with the amending direction:
   ...
   ```
   `Amends IDEA-MMM` trailers are same-repo by construction (the amending commit and the amended file live in one repo, so bare numbers are unambiguous here). A genuinely cross-repo amendment *narrative* — prose describing repo A's change tracking repo B's idea — spells the foreign ref `IDEA-NNN:project` per [`CROSS_PROJECT_IDEA_REFS`](../skills/idea/references/CROSS_PROJECT_IDEA_REFS.md).
