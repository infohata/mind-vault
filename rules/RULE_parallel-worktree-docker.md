...
| Django `sync_domains` DNS gate silently drops the production DOMAIN (cert issues, nginx serves no HTTPS vhost) on a fresh VPS | Debian installer added `127.0.1.1 <hostname>` to `/etc/hosts` when the machine hostname equals the domain; NSS (`files` + `myhostname`) returns loopback for the domain in every container, so `socket.getaddrinfo`-based gates mis-filter. HTTP-01 ACME works (nginx-relative), only HTTPS breaks after an apparently-successful bootstrap. | Make the gate query public DNS directly (dnspython + explicit `1.1.1.1` / `8.8.8.8`), not NSS; add a preflight check to deploy scripts that refuses when `getent hosts $DOMAIN` returns a loopback; change the hostname so it doesn't match DOMAIN. Pattern validated in teisutis IDEA-120 (2026-04-21). |
