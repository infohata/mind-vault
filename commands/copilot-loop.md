...
> **Failure mode — Copilot code review not enabled for the repo (2026-05-26 empirical).** Requesting Copilot as a reviewer *succeeds* (it appears in `/pulls/<N>/requested_reviewers` as login `Copilot`), but the repo/org doesn't have Copilot code review turned on — so Copilot is silently dropped from `requested_reviewers` shortly after, with **zero reviews, zero inline comments, and zero check-runs**. The signature is **assigned → vanishes → nothing posted**, distinct from the service-error mode (which DOES post a `COMMENTED` review). Detect: after a request, poll once; if Copilot is gone from `requested_reviewers` with no `/reviews`, no `/comments`, and no Copilot check-run, **re-request once** (via the API POST — `gh pr edit --add-reviewer @copilot` is a no-op here too) and poll again. If the second cycle is identical (assigned-then-dropped, nothing posted), **conclude code review is not enabled and hand back to the user immediately** — do NOT accumulate toward `max_idle_polls` (the feature won't turn on by waiting). Hand-back ask: enable Copilot code review in org settings (Settings → Copilot → Code review), use another engine (`/bugbot-loop`, `/ultrareview`), or merge without Copilot. Cheap project-memory note ("Copilot review not enabled for `<repo>`") saves the two-poll rediscovery next run.
