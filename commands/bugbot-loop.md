...
3. **Retrigger** — conditional on same-engine spacing:
   - If `last_bugbot_retrigger_at` is unset OR ≥5 min ago: fire `./tools/bugbot_retrigger.sh [PR_NUMBER]` (preferred — hard-codes the `bugbot run` body so it can be pre-approved in `~/.claude/settings.json` without risking arbitrary comment injection; falls back to current-branch PR lookup if no arg given; equivalent to `gh pr comment <PR> -b "bugbot run"` but auto-approved). Update `last_bugbot_retrigger_at` in the scratch file to the post-time. Clear `pending_retrigger` if set.
   - Else (last retrigger <5 min ago): **do NOT fire the script now.** Set `pending_retrigger=true` in the scratch file with the target PR number, then `ScheduleWakeup(delaySeconds=300, prompt="/bugbot-loop <PR>")`. Phase 4's pending-retrigger branch will fire the actual script on the next wake. Without this branch, a deferred retrigger orphans the push — the commit sits without a queued review while Phase 4 polls for comments that bugbot never produces.

   **Spacing rationale — ≥5 min between retriggers of the *same engine*, one retrigger per fix-cycle.** Cursor's check-suite queues bugbot reviews; rapid retriggers don't preempt a pending review, they STACK behind it. Field-observed degradation: 4 retriggers in 10 min stretched per-review latency from ~1-10 min (typical) to ~16 min as the queue worked through superseded entries. The spacing rule is **per-engine** — under dual-engine mode the post-batch bugbot retrigger and copilot retrigger hit independent queues, so firing both back-to-back after a single push (bugbot first, then copilot) does NOT violate spacing; the rule only restricts back-to-back bugbot retriggers (or back-to-back copilot retriggers). Discipline:
...
3. Decision tree — evaluate in order. The first two branches are **absolute hard-bound guards**: they are checked before any happy-path branch so that the happy paths (which are collectively exhaustive over wake state) cannot shadow them. **Both guards must flush a pending retrigger before handing back** — see the inline rule on each guard below. The third branch handles a deferred retrigger from a prior Phase 3 defer so that pushed-but-not-yet-retriggered commits don't get stranded mid-loop.
   - **Guard: active-work minutes ≥ 180** (excluding sleep) → before handing back, **if `pending_retrigger=true` and `last_bugbot_retrigger_at` is ≥5 min ago (or unset)**, fire `./tools/bugbot_retrigger.sh [PR_NUMBER]` once, update `last_bugbot_retrigger_at`, clear `pending_retrigger`. If `pending_retrigger=true` but still inside the spacing window (extremely rare given budget exhaustion implies long elapsed time), surface "PENDING BUGBOT RETRIGGER NOT FIRED — push orphaned at `<last_push_sha>`; run `./tools/bugbot_retrigger.sh <PR>` manually after 5 min from `last_bugbot_retrigger_at`" prominently in the hand-back report. Then hand back regardless of wake state.
   - **Guard: no-progress detector trips** — same finding category flagged 2× across cycles where a commit *attempted* that category (success-or-revert both count) → apply the same pending-retrigger flush rule as the active-work guard above (fire if spacing elapsed, else surface in hand-back), then hand back immediately. This closes the mixed-cycle stuck-loop case where a reverted fix could be retried indefinitely when a sibling finding's successful push re-triggered bugbot.
   - **Pending retrigger from prior defer** — `pending_retrigger=true` in the scratch file → **first check spacing**: if `last_bugbot_retrigger_at` is <5 min ago, the defer fired too early (Phase 4's first wake is 180s but the spacing window is 300s); **do NOT fire**, leave `pending_retrigger=true`, `ScheduleWakeup(delaySeconds=300 - elapsed_since_last_retrigger, prompt="/bugbot-loop <PR>")` to re-wake when the window genuinely elapses, and return without falling through to other branches. Otherwise (spacing elapsed): fire `./tools/bugbot_retrigger.sh [PR_NUMBER]`, update `last_bugbot_retrigger_at`, clear `pending_retrigger`, then continue evaluation (the retrigger doesn't immediately produce findings; fall through to the standard new-findings / clean-signal / idle-poll branches below for *this* wake's verdict). Skipping this branch leaves the push orphaned.
