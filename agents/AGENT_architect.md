...
- **One consumer over two payload shapes of the same logical resource half-works, silently.** When the same shared predicate, formatter or guard is fed by two endpoints/transports for one resource — a list route that post-processes and a raw pass-through, a v1 and a v2, a websocket frame and its REST twin — the fields it reads must be verified against BOTH payloads, not against the one the author had open. A guard reading `a || b` where one branch supplies only `a` and the other only `b` is not a guard on either; it fails OPEN on the payload nobody tested, and the shared code makes both call sites look covered by the same audit. Ask which field each producer actually emits, and prefer the field they have in COMMON — usually the one the richer payload was derived from — over the convenience object only one of them attaches.
...
- **A contract the UI will depend on is a load-bearing claim — demand the discriminating request, not the controller name.** A plan that adds paging/sorting/filtering UI, or keys a flow on a response field or status transition, cites the *response* of a probe whose result differs between "holds" and "doesn't" (`limit=5` on a list of 18 must return 5; `page=2` must differ from page 1). "The index goes through the paginate helper" is text about the system. The negative outcome must be a planned branch in the plan (annotate + file the backend item), not an executor improvisation (`skills/plan/references/PRODUCTION_PATH_VERIFICATION.md` § The backend contract is an axis too).
...
- **HTMX swap behavior** — partial-update correctness, scroll preservation, settle-state assertions.
