...
23. **Dropping entries from a const whitelist requires a view-side fallback to absorb stale-bookmark surprises** (UI / data-coupling, framework-agnostic): when tightening a whitelist const that's read by a view/template/dropdown — e.g. an enum→label map, an allowed-status list, a code→display-name table — verify that every consuming view has a render-fallback for codes that are about to be dropped. Without the fallback, a stale URL bookmark, a historical DB row, or a third-party API surfacing a "retired" code makes the UI silently render an empty cell, a blank `<option>`, or the literal-text key. With the fallback, the operator sees a raw code (`#42`, `status:unknown`, `tier:legacy`) and the issue surfaces loudly instead of degrading silently. PR #22 br-internal-panel cycle 0 (architect anticipated; Copilot would have flagged).

    Canonical shape (PHP/Blade, but the pattern is framework-agnostic): a controller passes `$billTypes = Invoice::BILL_TYPES` (a code→label const) to a Blade view. The view renders `{{ $billTypes[$invoice->bill_type] ?? '#'.$invoice->bill_type }}` for the list-row cell. When the plan drops entries 2 and 3 from `BILL_TYPES`, the existing `?? '#'.$invoice->bill_type` fallback catches any historical row with `bill_type IN (2, 3)` — instead of going blank, the cell renders `#2` / `#3` and the operator sees the surprise. Filter dropdowns that iterate the const directly (`@foreach ($billTypes as $code => $label)`) lose 2/3 as options — that's the correct behaviour for "those codes are no longer valid", but stale-bookmark URLs like `?bill_type=2` previously narrowed (whitelist accepted, filter applied, typically zero rows) and now broaden (whitelist rejects 2, filter is silently dropped, all rows return). The broaden-on-stale-bookmark failure mode is the louder degradation — operators notice "wait, that's everything" faster than "wait, that's nothing", so the const drop is generally correct AS LONG AS the per-row fallback is present.

    Drill-side checks while reviewing any PR that drops entries from a whitelist const read by views/templates:

    - **Grep for every consumer of the const** across views, controllers, serialisers, API responses: `grep -rn '<CONST_NAME>' --include='*.html' --include='*.blade.php' --include='*.jsx' --include='*.tsx' --include='*.vue' --include='*.py'`. Any consumer that does `const[$key]` / `const[key]` / `const.get(key)` without a default needs an explicit fallback for the dropped keys.
    - **Three valid fallback shapes**, in increasing robustness order: (a) inline `?? '<placeholder>'` / `|| '<placeholder>'` / `.get(key, '<placeholder>')` at the call site (covers the immediate row but is duplicated across templates if there are multiple consumers); (b) a normalised getter helper (`getBillTypeLabel($code)` returning `BILL_TYPES[$code] ?? "#$code"`) that every consumer uses (single source of truth); (c) defensive at the data-access boundary — when fetching the row, transform the unknown code into a sentinel that views can render without a key lookup.
    - **Filter / dropdown / whitelist gates**: separately, check if there's a `scopeFilter` / query-builder / API-validator that uses the const as a whitelist (`array_key_exists`, `if key in CONST`, `enum.has(value)`). When the const tightens, that gate naturally rejects the dropped codes — stale bookmarks become broaden-on-rejection failure mode (filter silently dropped, unfiltered list returns). Document this semantic change explicitly in the PR description so a reviewer doesn't read it as a regression.
    - **Test regression**: seed a row with one of the dropped codes (if the schema allows — bypass `Eloquent::save` constraints if needed); render the consuming view; assert the row appears with the fallback rendered (e.g. `assertSee('#2')` or whatever the fallback shape produces) and does NOT crash / render blank. Pin against a future refactor that drops the fallback "to clean up the view code".

    Cross-language shape catalogue:

    - **PHP/Blade**: `{{ $map[$key] ?? '#'.$key }}` per cell, OR a `Model::labelFor($code)` helper.
    - **Python/Django**: `{{ map|get_item:key|default:"#"|add:key }}` (with a `get_item` template tag), OR a model property `@property def label(self) -> str: return MAP.get(self.code, f"#{self.code}")`.
    - **JS/React**: `{map[code] ?? `#${code}`}` in JSX, OR a `useLabelFor(code)` hook.
    - **Go template**: `{{ index .Map .Key | default (printf "#%s" .Key) }}`, OR a helper func registered with the template.

    Distinguish: this finding is about **dropping known entries** from a stable whitelist. Distinct from finding #4 (CSS selector scope) and #14 (cross-surface state sync) which are about reads-vs-writes of shared state. Distinct from finding #6 (guard condition completeness) which is about ORs in conditional logic. The keyword that disambiguates: "the const used to have keys X and Y; the PR removes them; downstream consumers may still see those keys in the wild".
