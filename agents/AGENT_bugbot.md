...
14. **Bidirectional cross-surface state sync requires reciprocal writers**: when two surfaces share state via a bridge (e.g. localStorage `app_filters_org_<id>`, a shared cookie, an event bus), each surface must both *read* and *write* the bridge. Removing one surface's writer (refactor moves it from localStorage to server-session) silently breaks the other surface's reads — the unmodified surface keeps reading stale or empty bridge state.
   ...
   Surfaced in teisutis IDEA-136 [PR #409](https://github.com/infohata/teisutis/pull/409): list views' `unifiedFilterStorage.save()` calls were deleted; dashboard / chat / billing surfaces still called `unifiedFilterStorage.restore()` and `getStored()` (in files outside the PR diff) and silently degraded to stale-or-empty state. Bugbot's per-file review cleared the PR; the cross-surface regression surfaced in user smoke as "filters fork between dashboard and list views". The fix retired the bridge entirely + migrated readers to server session — phase 2 ([commit `1b2045d7`](https://github.com/infohata/teisutis/commit/1b2045d7) — chat surfaces).
