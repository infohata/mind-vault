...
- **Counter persistence** — the session commit counter, idle-poll counter, active-work-minutes tracker, `last_seen_bugbot_signal_id` (per-engine slot under the shared-core refactor; formerly `last_seen_bugbot_signal_id` in the pre-shared-core single-engine wrapper), `last_push_sha`, and no-progress detector state (per-category commit-attempt counts) must be checkpointed to the scratch file after *every* mutation. **Path under the shared review-loop core (post-IDEA-005)**: `~/.claude/memory/projects/<slug>/review-loop-pr-<N>.md` (engine-agnostic; all engines' state in one file). The legacy per-engine path `bugbot-pr-<N>.md` is retained here for historical context; new sessions write to the shared path. Conversation context can be summarised away across wake cycles; counters that live only in-context make the hard bounds unenforceable.
